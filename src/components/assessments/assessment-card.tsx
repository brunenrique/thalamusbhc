...
if (!assessment) {
return ( <div className="p-4 text-sm text-muted-foreground">
Dados indisponíveis </div>
);
}
...
return ( <Card className="shadow-sm hover:shadow-md transition-shadow flex flex-col h-full"> <CardHeader> <div className="flex justify-between items-start"> <CardTitle className="font-headline text-lg">
{assessment.name} </CardTitle> <Badge variant={getStatusBadgeVariant()} className="capitalize">
{getStatusIcon()} <span className="ml-1.5">
{statusLabels\[assessment.status]} </span> </Badge> </div>
{assessment.description && ( <CardDescription className="text-xs mt-1">
{assessment.description} </CardDescription>
)} </CardHeader>
